...
bool IsDesktopBackgroundWindow(HWND hwnd) {
    ...
    // 兼容 Win10 和 Win11 的桌面背景窗口类名
    // Win11 中可能会出现 "DesktopWallpaper" 或新的 WorkerW 结构
    if (classNameStr == L"WorkerW" || classNameStr == L"DesktopWallpaper") {
        // 进一步验证：检查是否包含 SHELLDLL_DefView (图标层)
...
// 定义 WS_EX_NOREDIRECTIONBITMAP (如果编译器头文件中没有)
#ifndef WS_EX_NOREDIRECTIONBITMAP
#define WS_EX_NOREDIRECTIONBITMAP 0x00100000
#endif

HWND FindModernWorkerW() {
    HWND progman = FindWindowW(L"Progman", nullptr);
    if (!progman) return nullptr;

    // 1. 检查是否是 Win11 的 "Raised Desktop" 模式
    // 获取 Progman 的扩展样式
    LONG exStyle = GetWindowLongW(progman, GWL_EXSTYLE);
    bool isRaisedDesktop = (exStyle & WS_EX_NOREDIRECTIONBITMAP) != 0;

    HWND workerW = nullptr;
    HWND shellDllDefView = nullptr;

    // 2. 触发 WorkerW 生成 (经典魔术指令，必须保留)
    SendMessageTimeoutW(progman, 0x052C, 0x0000000D, 0x00000001, SMTO_NORMAL, 1000, nullptr);

    if (isRaisedDesktop) {
        // 【Win11 新逻辑】
        // 在 Raised Desktop 模式下，WorkerW 是 Progman 的直接子窗口
        // 结构如下：
        // Progman
        //   └── SHELLDLL_DefView
        //   └── WorkerW  <-- 这就是我们要找的壁纸层
        ...
        // 【Win10 及旧版逻辑】
        // 遍历顶级窗口，找到包含 SHELLDLL_DefView 的 WorkerW，然后取其下一个兄弟节点
        HWND currentWorkerW = nullptr;
        while ((currentWorkerW = FindWindowExW(nullptr, currentWorkerW, L"WorkerW", nullptr)) != nullptr) {
            shellDllDefView = FindWindowExW(currentWorkerW, nullptr, L"SHELLDLL_DefView", nullptr);
            if (shellDllDefView != nullptr) {
                // 找到包含图标的 WorkerW，获取它的下一个兄弟节点
                workerW = FindWindowExW(nullptr, currentWorkerW, L"WorkerW", nullptr);
                break;
            }
        }
    }
...
void attach(const Napi::CallbackInfo &info) {
    ...
    // }
    if (workerW == nullptr) {
        // 在 Win11 中，如果找不到 WorkerW，可能是因为系统使用了全新的 DWM 组合模式
        // 这里可以添加一个回退方案：尝试嵌入到 Progman 中
        workerW = FindWindowW(L"Progman", nullptr);
        if (workerW == nullptr) {
            Napi::TypeError::New(env, "Couldn't locate WorkerW or Progman (Win11 compatibility issue)").ThrowAsJavaScriptException();
            return;
        }
    }
